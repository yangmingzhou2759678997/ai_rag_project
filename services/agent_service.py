...
async def process_chat_request(
    ...
    try:
        ...
        final_messages = [
            {
                ...
                "content": (
                    ...
                    "【强制规则 6】：如果知识库资料中包含“【来源文件：文件名】”，回答正文结束后必须增加“参考来源”部分，只列出本次回答实际使用的来源文件名；相同文件只列一次，禁止捏造不存在的来源！\n"
                    "【强制规则 7】：只回答最新的重写问题；历史消息仅用于理解指代，不得无必要重复上一轮答案！\n"
                    "【强制规则 8】：用户明确限定文件、文件类型、实体、项目、阶段、日期或章节时，只能采用与该范围匹配的工具资料，不得混用其他文件或其他对象的规则！\n"
                    "【强制规则 9】：问题要求完整流程、全部步骤、需要记录和复核哪些内容等完整列举时，必须整合工具结果中与目标直接相关的全部片段，包括精确配置值、校验词、操作步骤和复核要求，不能只摘取一部分！\n"
                    "【强制规则 10】：工具结果包含多个来源且规则互相冲突时，以用户指定的实体、文件、阶段或章节为准，不得把不同来源内容拼接成一套规则！\n"
                    "【强制规则 11】：当用户明确指定实体、项目、系统、平台或业务名称时，先确认哪些来源文件的至少一个资料片段明确出现该实体名称，只使用这些已确认来源中的相关片段；同一确认来源的其他片段即使没有重复实体名称，也可以补全同一流程；必须忽略其他未确认来源中的相似流程，禁止仅因发布、归档、复核、维护等动作相似就拼接不同实体规则；参考来源只能列出实际采用且通过实体范围核对的文件！\n"
        ...
        logger.opt(exception=True).error(" [Agent大脑] 智能体中枢崩溃: {}", e)
        raise
