...
class TestTextPdfParsing(unittest.TestCase):
    def test_txt_contains_body_and_final_paragraph(self):
        """验证TXT普通正文和最后一段都能提取"""
        text = extract_document("rag_eval_basic.txt", read_fixture("rag_eval_basic.txt"))
    ...
    def test_markdown_contains_heading_and_lists(self):
        ...
        text = extract_document("rag_eval_technical.md", read_fixture("rag_eval_technical.md"))
    ...
    def test_pdf_contains_cross_page_evidence(self):
        """验证PDF跨页关联事实的两部分都能提取"""
        text = extract_document("rag_eval_pages.pdf", read_fixture("rag_eval_pages.pdf"))
    ...
    def test_all_fixtures_generate_chunks(self):
        ...
        for file_name in file_names:
            text = extract_document(file_name, read_fixture(file_name))
